...
def modelo(theta, z, dz, zdim):
    ...
    try:
        ...
        0
    return 5*torch.log(dl + 1e-300)/np.log(10) + 43.
...
class Potential:
  ...
  def gradi(self, theta):
    ...
    #gradient, = torch.autograd.grad(val, theta_grad, grad_outputs=torch.ones(), create_graph=True)
    gradient, = torch.autograd.grad(val, theta_grad, grad_outputs=torch.cuda.FloatTensor(batch_size).fill_(1), create_graph=True)
    return gradient
...
class Prior:
  def __init__(self, dist=None, low=None, high=None, mean=None, cov=None):
    ...
    if dist==None:
        ...
        #elf.pdf = torch.ones(batch_size).float()/vol
        self.pdf = torch.cuda.FloatTensor(batch_size).fill_(1)/vol.cuda()
  ...
  def get_samples(self, n):
    ...
    if self.dist=="normal":
        ...
        return  torch.tensor(np.random.uniform(low=low.numpy(), high=high.numpy(), size=(n, low.shape[0]))).float().cuda()
...
class Leapfrog:
    ...
    def __init__(self, U, m, ndim, nnx, nnv, e, b, reg, sc, lr, msup, minf):
        ...
        self.xi = torch.tensor(self.U.prior.get_samples(batch_size)).float().cuda()
        self.reg = reg
        self.sc = sc
        self.lr = lr
        ...
        self.chain = []
        self.train_chain = []
        self.train_chain.append(self.xi.cpu().numpy())
        self.msup = msup
        self.minf = minf
        self.beta = 5
        self.ARate = 0
    ...
    def dyn(self, x, v):
        ...
        for i in range(self.m):
            mask = torch.tensor(np.random.choice([1., 0.], size=(self.b, self.ndim), p=[0.5, 0.5])).float().cuda()
            nmask = 1 - mask
    ...
    def time_encoding(self, mi):
        ...
        return torch.tensor(val).float().cuda()     
    ...
    def train(self):
        ...
        self.xi = torch.tensor(accepted_sample).float().cuda()
    ...
    def evolve(self, initial_sample):
    	if self.learning_state==0:
    		self.xi = initial_sample
    		self.learning_state = 1
    		self.chain.append(initial_sample.numpy())
    	xnext, vnext, jacp = self.dyn(self.xi)
    	Ap, ASam, accepted = acceptance_prob(self.xi, xnext, self.vi, vnext, jacp, self.U, minf=self.minf, msup=self.msup)
...
class Chi2Evaluator():
	def __init__(self, data, cov, z):
		...
		zc = z.cpu().numpy()
		zc = np.insert(zc, 0, 0)
		self.dz = torch.tensor(zc[1:] - zc[:-1]).cuda()
		self.zdim = z.shape[0]


	def calculateAndPlot(self, chain):
		ii, jj, _ = chain.shape
		...
		for j in range(jj)[:5]:
			try:
				plt.plot(self.Cn[:, j], label="chain {}".format(j))
			except:
				0
		plt.legend()
		plt.savefig("chi2.png", dpi=dpi)
		return


def PostProcessing(_loss, _accepted, chi2eval, _save):
    _loss = np.array(_loss)
    _accepted = np.array(_accepted)
    chain = torch.Tensor(leapfrog.train_chain).numpy()
    P = chain
    ts = datetime.datetime.now().strftime("%Y_%m_%d_%H_%M_%S")
    if _save:
        np.save("samples_train_{}".format(ts), P)
        np.save("loss_train_{}".format(ts), _loss)
        np.save("acceptance_rate_train_{}".format(ts), _accepted)

    chi2eval.calculateAndPlot(torch.Tensor(chain))

    plt.clf()
    for i in range(5):
    	plt.scatter(P[:, i, 0], P[:, i, 1], marker=".", alpha=0.1)
    plt.xlim([0, 1])
    plt.ylim([0, 1])
    ...
    plt.clf()
    for i in range(5):
    	plt.scatter(P[:, i, 0], P[:, i, 2], marker=".", alpha=0.1)
    plt.xlim([0, 1])
    plt.ylim([-6, -1/3])
    plt.xlabel(r"$\Omega_{m}$")
    plt.ylabel(r"w")
    ...
    plt.clf()
    for i in range(5):
    	plt.scatter(P[:, i, 1], P[:, i, 2], marker=".", alpha=0.1)
    plt.xlim([0, 1])
    plt.ylim([-6, -1/3])
...
if __name__=="__main__":
	...
	direc = "/home/mauricio/Documents/Uni/primavera_2018/cosmo/gal.txt"
	...
	redshift = torch.tensor(redshift).float().cuda()
	mu_obs = torch.tensor(mu_obs).float().cuda()
	cov = torch.tensor(cov).float().cuda()
	...
	for j in range(Nt):
		try:
			...
			tf = time.time()
			if j%10==0:
				verboseprint("Train iteration {} time {:.2f} Loss {:.2f} Acceptance ratio {:.2f}".format(j, tf - ti, Losses[j], torch.mean(accepte[:, 0])/(j + 1)))#, object(), 3)
			if plott:
