...
def modelo(theta, z, dz, zdim):
	...
	try:
	    dl[(ok<1e-5)*(ok>0)] = f1[(ok<1e-5)*(ok>0)]
	except:
	    0
	try:
	    dl[(ok>-1e-5)*(ok<0)] = f1[(ok>-1e-5)*(ok<0)]
	    ...
	    0
	try:
	    dl[ok>1e-5] = f2[ok>1e-5]

	except:
	    0
	try:
	    dl[ok<-1e-5] = f3[ok<-1e-5]
	except:
	    0
...
def check(_theta, _v):
    ...
    args1 = _theta[:, 1]<0.
    args2 = _theta[:, 2]>-1/3
    _theta[args0, 0] = _theta[args0, 0]*-1
    _theta[args1, 1] = _theta[args1, 1]*-1
    _theta[args2, 2] = -1/3 - _theta[args2, 2]

    _v[args0, 0] = _v[args0, 0]*-1
    _v[args0, 1] = _v[args0, 1]*-1
    _v[args0, 2] = _v[args0, 2]*-1
...
if __name__=="__main__":
    ...
    N = 10
    theta = pri.get_samples(N)
    p = torch.randn((N, 3)).float()
    theta[0] = torch.Tensor([-0.29, -0.65, -1.])
    """
