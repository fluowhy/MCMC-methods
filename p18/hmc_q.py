...
def model(theta, z, dz, zdim):
	...
	o_k_s = torch.sqrt(torch.abs(omega_k)).view(-1, 1).repeat(1, zdim).float()
	argsin = o_k_s*I
	dl = (1 + z)*((may*torch.sinh(argsin) + men*torch.sin(argsin))/(o_k_s + 1e-300) + eq*I)
	# integral
	return 5*torch.log(dl + 1e-300)/np.log(10), dl
"""
...
class Leapfrog:
    ...
    def __init__(self, U, m, ndim, nnx, nnv, e, b, reg, sc, lr, msup, minf):
        ...
        self.qi = torch.tensor(self.U.prior.get_samples(batch_size)).float()
        self.reg = reg
        self.sc = sc
        self.lr = lr
    ...
    def train(self):
        ...
        qnext, qvnext, qjacp = self.dyn(self.qi, qvi)
        self.nnx.zero_grad()
        self.nnv.zero_grad()
        Ap = AcceptanceProbability(self.xi, xnext, vi, vnext, self.U, jacp, minf=self.minf, msup=self.msup)
        Aq = AcceptanceProbability(self.qi, qnext, qvi, qvnext, self.U, qjacp, minf=self.minf, msup=self.msup)
        L = loss(self.xi, xnext, self.qi, qnext, Ap, Aq, self.sc, self.reg)
        #L = loss2(self.train_chain, ASam, 1) + self.beta*(1 - torch.sum(accepted)/self.ndim/self.b)
        #L = loss3(self.pi, pnext) + self.beta*(1 - torch.sum(accepted)/self.ndim/self.b)
        L.backward()
...
def loss(state0x, state1x, state0q, state1q, apx, apq, sc, reg):
    """
    pp, pq: probabilities
    ...
    """
    dx = distance(state0x, state1x)
    dq = distance(state0q, state1q)
    return torch.mean(lam(sc, dx, apx)) + torch.mean(reg*lam(sc, dq, apq))
...
if __name__=="__main__":
    ...
    """
    state = torch.Tensor([[0.3, 0.7, -1.0], [0.4, 0.6, -1.1]])
    zc = redshift.numpy()
    zc = np.insert(zc, 0, 0)
    dz = torch.tensor(zc[1:] - zc[:-1])
    zdim = redshift.shape[0]

    mu, dl =  modelo(state, redshift, dz, zdim)

    plt.plot(redshift.numpy(), mu[0].numpy(), label=r"$\mu$")
    plt.plot(redshift.numpy(), dl[0].numpy(), label=r"$d_{l}$")
    plt.legend()
    plt.show()

    L = Likelihood(mu_obs, cov)
    chi2 = L.get_likelihood(mu)/-0.5
    """
    ...
    for j in range(Nt):
        try:
            ...
            tf = time.time()
            verboseprint("Train iteration {} time {:.2f} Loss {:.2f} Acceptance ratio {:.2f}".format(j, tf - ti, Losses[j], torch.mean(accepte[:, 0])/j))#, object(), 3)
            if plott:
