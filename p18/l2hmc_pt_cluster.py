...
class MLP(torch.nn.Module):
    ...
    def forward(self, x, v, t):
        ...
        T = self.t(out)
        epsilon = self.sigmoid(self.epsi(out))*1e-2
        return S, Q, T, epsilon
...
class Leapfrog:
    ...
    def back_dyn_fun(self, x, v, t, mask, nmask):
        ...
        self.Sv1, Qv, Tv, self.e = self.nnv.forward(x, gradient, t)
        v = v*torch.exp(- 0.5*self.Sv1*self.e) + 0.5*self.e*(gradient*torch.exp(self.e*Qv) + Tv)        
        self.Sx2, Qx, Tx, self.e = self.nnx.forward(mask*x, v, t)
        x = x*mask + nmask*(x*torch.exp(- self.e*self.Sx2) - self.e*(v*torch.exp(self.e*Qx) + Tx))        
        self.Sx3, Qx, Tx, self.e = self.nnx.forward(nmask*x, v, t)
    ...
    def dyn(self, x):
        ...
        for i in range(self.m):
            ...
            if self.d==1:
                x = self.for_dyn_fun(x, v, t, mask, nmask)
            elif self.d==-1:
                x = self.back_dyn_fun(x, v, t, mask, nmask)
...
for j in range(Nt):
	...
	#tf = time.time()
	#print("Train iteracion {} tiempo {}".format(j, tf - ti))
...
for j in range(Ne):
	...
	Accepted.append((torch.sum(accepted, dim=0)[0].numpy()))
	#tf = time.time()
	#print("Eval iteracion {} tiempo {}".format(j, tf - ti))
Accepted = np.array(Accepted)
